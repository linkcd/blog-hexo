...
2. The granularity of data has much more detailed scale in IoT and Big Data world. One example is, tracking every single piece of coal from a carrier ship sounds crazy, but tracking every data signal from thousands of sensors from the very same ship is quite common. 
...
The data source can, of course, publish all individual addresses for all messages, but it will be too many. As far as consumers have the channel root address, they can go through all addresses from the root, to find the specific address/message to verify. See step 5.
...
Data source sends above json object to the MAM channel (IOTA tangle). The json object will be stored in a MAM address inside of the channel. This can be done by using the demo code from above.

**--- At this point, data source completes all needed tasks----**

### Step 4. Data consumer obtains the root address of MAM channel from data source
...
Data consumer go through the MAM channel, address by address, to find the json object for the specific package, by using packageID. Then it obtains the **wayOfProof** and **valueOfProof** for that package.
...
Now we extend the format to include an optional field **inputs**, which is an array of MAM addresses. These addresses represent the data integrity information of all inputs of the current data package.  A MAM address is a global unique identifier in Tangle, regardless which channel it belongs to.
...
**Q2**: Can I use IOTA and MAM protocol without hosting a IOTA node?
...
**Q3**: What is Public nodes and Permanode? Which one I should use?
...
**A4**: Yes, you can simply create a seed (a string) locally and store data into MAM channel. Feel free to generate seeds your sensors/applications.

**Q5**: I do not want to use public MAM channels that anyone can take a look, even i know the messages only contain hash values. How can i protect the channels?
...
**Q6**: I have an application that is sending out data to consumers. Do I need to do anything if there is a new consumer start using my data and build the lineage on top of it?
**A6**: No. As a data source sitting in the upstream, you do not need to do anything for downstream consumers.

**Q7**: I am a data consumer. What information do I need to create the whole data lineage that covers all inputs in different levels? For example, if the data flow is Alice->Bob->Carol->myself, do I need to know the MAM root address of Alice, Bob and Carol?
**A7**: No, you only need MAM root of Carol. As far as you follow the input fields recursive，you can check integrity and lineage of Bob (Carol's upstream) and Alice (Bob's upstream). In above insurance case, the insurance company can also follow the inputs fields to check, for example, the translator's message and the Norwegian police station's message.
...
# Summary
In this article, we discussed the detailed design of verification data schema, such as these crucial fields: **"datapackageId"**, **"wayofProof"** , **"valueOfProof"** and **"inputs"**. We also implemented the solution on  the selected DLT: IOTA and its MAM protocol. In next article of this series, we will put them into actions, and have closer look at some components of the DILAAS. 


# Appendix #
Masked Authenticated Messaging (MAM) was introduced by IOTA in Nov 2017. The high level description can be found at [here](https://blog.iota.org/introducing-masked-authenticated-messaging-e55c1822d50e). Besides, some deep dive information of Tangle transaction and MAM can be found at:
